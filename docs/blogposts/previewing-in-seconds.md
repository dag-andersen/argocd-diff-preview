...
|----------|----------|------|------|
| **Ephemeral clusters** | Getting started, maximum security | • Simple setup<br>• Complete isolation<br>• Works with any CI/CD | • Slow (60-90s overhead) |
| **Existing cluster** | Teams prioritizing speed | • Fast <br>• Same accuracy<br>• Leverages Argo CD caching | • Requires dedicated Argo CD setup<br>• Need cluster credentials in CI/CD<br>• Infrastructure maintenance |
| **Existing cluster + self-hosted runner** | Teams prioritizing speed AND security | • Fast + secure<br>• Leverages Argo CD caching<br>• No credentials in CI/CD | • Most complex setup<br>• GitHub Actions only |
