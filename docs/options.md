...
| `--repo-server-address <address>`                         | `REPO_SERVER_ADDRESS`                      | -                 | Address of the Argo CD repo server (host:port). Connects directly instead of port-forwarding. Requires --render-method=repo-server-api"                                                        |
