...
func TestFormatNewFileDiff(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			output := formatNewFileDiff(tt.content, tt.contextLines, tt.ignorePattern, nil).content
...
func TestFormatDeletedFileDiff(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			output := formatDeletedFileDiff(tt.content, tt.contextLines, tt.ignorePattern, nil).content
			if output != tt.expectedOutput {
				t.Errorf("Expected:\n%s\nGot:\n%s", tt.expectedOutput, output)
			}
		})
...
func TestFormatModifiedFileDiff(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			output := formatModifiedFileDiff(tt.oldContent, tt.newContent, tt.contextLines, tt.ignorePattern, nil).content
			if output != tt.expectedOutput {
				t.Errorf("Expected:\n%s\nGot:\n%s", tt.expectedOutput, output)
			}
		})
...
func TestFormatDiffEdgeCases(t *testing.T) {
	...
	if output := formatNewFileDiff(specialChars, 3, nil, nil).content; !strings.Contains(output, "+line with spaces") ||
	...
	if output := formatNewFileDiff(trailingNewlines, 3, nil, nil).content; output != expectedTrailingOutput {
	...
	output := formatModifiedFileDiff(oldContent, newContent, 3, &ignorePattern, nil).content
	if output != "" {
...
func TestContextLinesVariations(t *testing.T) {
	...
	zeroContextOutput := formatModifiedFileDiff(oldContent, newContent, 0, nil, nil).content
	...
	largeContextOutput := formatModifiedFileDiff(oldContent, newContent, 100, nil, nil).content
	...
	smallContextOutput := formatModifiedFileDiff(oldContent, newContent, 2, nil, nil).content
...
func TestIgnorePatternEdgeCases(t *testing.T) {
	...
	emptyPattern := ""
	output := formatNewFileDiff(content, 3, &emptyPattern, nil).content
	if output != "+line1\n+IGNORE_line2\n+line3\n" {
		t.Errorf("Empty ignore pattern not handled correctly: %s", output)
	}

	// Test with nil ignore pattern
	nilOutput := formatNewFileDiff(content, 3, nil, nil).content
	if nilOutput != "+line1\n+IGNORE_line2\n+line3\n" {
	...
	invalidOutput := formatNewFileDiff(content, 3, &invalidRegex, nil).content
...
func TestLargeDiff(t *testing.T) {
	...
	output := formatModifiedFileDiff(oldContent, newContent, 2, nil, nil).content
	...
	controlledOutput := formatModifiedFileDiff(oldContent, newContent, 1, nil, nil).content
	expected := " common2\n-this will change\n+this was changed\n common4\n"
...
func TestRegexIgnorePatterns(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"_modified", func(t *testing.T) {
			...
			pattern := tt.pattern
			output := formatModifiedFileDiff(oldContent, newContent, 3, &pattern, nil).content
	...
	newOutput := formatNewFileDiff(testerContent, 3, &testerPattern, nil).content
	if !strings.Contains(newOutput, "+TEST_LINE") {
		t.Errorf("New file output should contain TEST_LINE: %s", newOutput)
	}

	delOutput := formatDeletedFileDiff(testerContent, 3, &testerPattern, nil).content
	...
	for _, tt := range simplePatterns {
		...
		t.Run(tt.name+"_new", func(t *testing.T) {
			output := formatNewFileDiff(content, 3, &pattern, nil).content
		...
		t.Run(tt.name+"_deleted", func(t *testing.T) {
			output := formatDeletedFileDiff(content, 3, &pattern, nil).content
			if !strings.Contains(output, "-"+tt.input) {
...
func TestMixedChangesWithRegex(t *testing.T) {
	...
	output := formatModifiedFileDiff(oldContent, newContent, 1, &pattern, nil).content
...
+  config: value2`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var output string
			if tt.oldContent == "" {
				output = formatNewFileDiff(tt.newContent, tt.contextLines, tt.ignorePattern, nil).content
			} else if tt.newContent == "" {
				output = formatDeletedFileDiff(tt.oldContent, tt.contextLines, tt.ignorePattern, nil).content
			} else {
				output = formatModifiedFileDiff(tt.oldContent, tt.newContent, tt.contextLines, tt.ignorePattern, nil).content
			}
...
func TestInvalidRegexPatterns(t *testing.T) {
	...
	for _, pattern := range invalidPatterns {
		t.Run("Invalid_"+pattern, func(t *testing.T) {
			...
			output := formatNewFileDiff(content, 3, patternPtr, nil).content
			...
			newContent := "line1\nTIMESTAMP: 654321\nline3"
			modOutput := formatModifiedFileDiff(content, newContent, 3, patternPtr, nil).content
