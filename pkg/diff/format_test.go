...
// blocksContent is a helper to get raw content from ResourceBlocks (for tests that pass nil resourceIndex)
func blocksContent(blocks []ResourceBlock) string {
	var result strings.Builder
	for _, block := range blocks {
		// For tests without resourceIndex, blocks have empty headers and just raw content
