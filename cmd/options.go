...
and generates a diff of the resources that would be applied.`,
	...
	rootCmd.Flags().Uint64("timeout", 180, "Set timeout in seconds")
	...
	rootCmd.Flags().UintP("line-count", "c", 10, "Generate diffs with <n> lines of context")
	...
	// Other options
	rootCmd.Flags().Uint("max-diff-length", 65536, "Max diff message character count")
