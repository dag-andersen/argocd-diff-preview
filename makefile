...
local-test-cargo: pull-repostory
	cargo run -- -b "$(base_branch)" -t "$(target_branch)" --repo $(github_org)/$(gitops_repo) -r "$(regex)" --debug --diff-ignore "$(diff-ignore)" --timeout $(timeout) --kustomize-build-options="$(kustomize-build-options)" -l "$(selector)"
