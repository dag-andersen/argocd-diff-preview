...
// gitops-engine is a nested, untagged module inside the argo-cd repo (argo-cd
// itself uses `replace => ./gitops-engine`, which doesn't apply to consumers).
// Pin it to the gitops-engine tree at the argo-cd release tag we require above
// (this pseudo-version is the v3.5.1 tag commit); update it together with the
// argo-cd bump.
...
// k8s.io/kubernetes requires its staging modules at v0.0.0; pin them all to
// the staging release matching the k8s.io/kubernetes minor argo-cd pins
// (mirrors argo-cd's own replace block). Update together with argo-cd bumps.
...
replace k8s.io/legacy-cloud-providers => k8s.io/legacy-cloud-providers v0.36.1
